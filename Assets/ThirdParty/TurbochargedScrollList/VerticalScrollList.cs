...
namespace Jing.TurbochargedScrollList
{
    ...
    public class VerticalScrollList : BaseScrollList
    {
        ...
        protected override void ResizeContent(UpdateData updateConfig)
        {
            ...
            // 如果是居中对齐，调整内容位置
            if (layout.alignment == VerticalLayoutSettings.Alignment.Center)
            {
                float contentHeight = content.rect.height;
                float viewportHeight = viewportSize.y;
                if (contentHeight < viewportHeight)
                {
                    float offset = (viewportHeight - contentHeight) / 2;
        ...
        protected override void Refresh(UpdateData updateConfig, out int lastStartIndex)
        {
            ...
            if (layout.alignment == VerticalLayoutSettings.Alignment.Center)
            {
                float totalContentHeight = -itemY - layout.gap + layout.paddingTop + layout.paddingBottom;
                float viewportHeight = viewportSize.y;
                if (totalContentHeight < viewportHeight)
                {
                    float offset = (viewportHeight - totalContentHeight) / 2;
                    content.localPosition = new Vector3(content.localPosition.x, -offset, content.localPosition.z);
                }
            }
        }
