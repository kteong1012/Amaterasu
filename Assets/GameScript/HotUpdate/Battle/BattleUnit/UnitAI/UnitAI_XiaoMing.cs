...
namespace Game
{
    public class UnitAI_XiaoMing : UnitAI
    {
        ...
        public override void Act(UnitAIComponent aIComponent)
        {
            ...
            var currentTarget = GameServices.BattleUnitService.GetUnit(_currentTargetId);
            // 如果当前目标为空或者死亡, 重新选择目标
            if (currentTarget == null || currentTarget is BattleUnitController battleUnit && battleUnit.IsDead())
            {
                currentTarget = GameServices.BattleUnitService.GetNearestEnemy(battleUnitController);
            }
