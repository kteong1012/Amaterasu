...
namespace Game
{
    ...
    public partial class BattleRoomService : GameService
    {
        ...
        {
            SSS.Get<BattleUnitService>().DestroyAllUnits();
            var unit1 = SSS.Get<BattleUnitService>().CreateUnit(1001, 10, UnitCamp.Enemy);
            unit1.transform.position = new Vector3(20, unit1.transform.position.y, 20);
            var unit2 = SSS.Get<BattleUnitService>().CreateUnit(1001, 10, UnitCamp.Player);
        ...
        private async void OnBattleEnd(IEventMessage message)
        {
            ...
            var uiBattleEndPanel = SSS.Get<UIService>().OpenPanel<UIBattleEndPanel>();
