...
public static class AnalyzerHelper
{
    ...
    public static string GetNameSpace(this INamedTypeSymbol namedTypeSymbol)
    {
        ...
        while (namespaceSymbol.ContainingNamespace != null)
        {
            ...
            if (namespaceSymbol.IsGlobalNamespace || string.IsNullOrEmpty(namespaceSymbol.Name))
            {
                break;
            }
    ...
    public static void AddSourceOrWriteFile(this GeneratorExecutionContext context, string hintName, string source)
    {
        var folder = Path.Combine(Environment.CurrentDirectory, "Generated");
#if GENERATE_FILE
        if (!Directory.Exists(folder))
        {
            Directory.CreateDirectory(folder);
        }
        File.WriteAllText(Path.Combine(folder, hintName), source);
#else
        if (Directory.Exists(folder))
        {
