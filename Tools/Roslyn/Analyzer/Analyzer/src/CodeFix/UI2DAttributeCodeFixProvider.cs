...
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(UI2DAttributeCodeFixProvider)), Shared]
    public class UI2DAttributeCodeFixProvider : CodeFixProvider
    {
        public override ImmutableArray<string> FixableDiagnosticIds => ImmutableArray.Create(DiagnosticIds.UI2DAttribute);
        ...
        public async override Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            var action = CodeAction.Create(
                title: "Add UI2DAttribute",
                createChangedDocument: c => AddUI2DAttributeAsync(context.Document, classDeclaration, UI2DAttributeAnalyzer.UI2DAttributeName, c),
                equivalenceKey: nameof(UI2DAttributeCodeFixProvider));

            // 注册codeFix Code Action
            context.RegisterCodeFix(action, diagnostic);
        }

        private async Task<Document> AddUI2DAttributeAsync(Document document, ClassDeclarationSyntax classDeclaration, string attributeName, CancellationToken c)
        {
            ...
            // 构造新的Attribute,省略Attribute后缀, 添加UI2DAttributeAnalyzer.UI2DAttributeDefaultParam参数
            var attributeArgument = SyntaxFactory.ParseExpression(UI2DAttributeAnalyzer.UI2DAttributeDefaultParam);
